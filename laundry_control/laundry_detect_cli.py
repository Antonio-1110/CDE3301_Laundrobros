...
laundry clusters, and optionally publish the flagged deviation
...
def build_parser():
    ...
    parser.add_argument(
        ...
        help=(
            "Minimum nearest-neighbor distance (metres) from the "
            f"baseline for a point to count as a deviation "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Publish the flagged (pre-cluster) deviation points "
...
def publish_deviations(candidate_csv, baseline_csv, args, clusters):
    """
    Publish the raw flagged (pre-cluster) deviation points, not the
    clusters, so the deviation mask itself can be sanity-checked
    (e.g. to judge if --threshold needs adjusting) independently of
    ...
    from .laundry_detect import compute_deviation
    ...
    class DeviationPublisherNode(Node):
        ...
        def publish_cloud(self):
            ...
            self.get_logger().info(
                f"Published {deviating_xyz.shape[0]} deviation "
                f"point(s) on '{args.topic}' (TRANSIENT_LOCAL - "
                "late RViz subscribers will still see it)."
            )
