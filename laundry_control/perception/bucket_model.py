...
"""
A fitted geometric model of the empty bucket, plus its learned noise field.

The residual/noise field is what turns the geometric model into a
detection reference (see perception/detect.py).

WHY THIS EXISTS
---------------
The detector used to diff a scan against a single baseline
...
one-3cm-cell problem documented in perception/detect.py, and it is
...
a ~25 degree cone field of view (config.TOF_FIELD_OF_VIEW_RAD), so its reading is
...
rotation (config.TOF_SENSOR_OFFSET_*), so any boresight error shows up as a
...
# `laundry scan` run is ~50s at 20Hz, so about 1000 points per scan and
